...
# Simple ping scan - just checks if host is up
nmap -sn 192.168.1.1

# Ping scan entire subnet
nmap -sn 192.168.1.0/24

# DNS resolution only (no port scan)
nmap -sL 192.168.1.0/24

# SYN scan on common ports (read-only, no connection completed)
nmap -sS 192.168.1.1

# Scan with no ping (assume host is up)
nmap -Pn 192.168.1.1

# Only show open ports quietly
...
# Full TCP connect scan (completes 3-way handshake)
nmap -sT 192.168.1.1
...
# UDP scan (can cause service disruption due to ICMP rate limiting)
nmap -sU 192.168.1.1

# Intrusive + version intensity max
...
# Slowloris DoS vulnerability check
...
# Check for anonymous FTP + list files
nmap -p 21 --script ftp-anon,ftp-syst 192.168.1.1

# SMB null/anonymous session check
...
# SNMP full walk (network info, interfaces, processes)
...
# HTTP headers, methods, and page title gathering
...
# MySQL diagnostic metadata
nmap -p 3306 --script mysql-info,mysql-databases 192.168.1.1

# Run all discovery-class scripts at once
...
# Fragment packets (splits TCP headers over several packets to bypass simple firewalls)
nmap -f 192.168.1.1

# Specify a custom MTU (Maximum Transmission Unit) - must be a multiple of 8
nmap --mtu 24 192.168.1.1

# Spoof Source Port (forces scan to come from a common trusted port, e.g., DNS port 53)
nmap -g 53 192.168.1.1

# Spoof Source IP (makes the target think the scan is coming from a different address)
nmap -S 10.0.0.5 -e eth0 192.168.1.1

# Decoy Scan (mixes your IP with random spoofed decoy IPs so defenders can't tell who started it)
nmap -D RND:10 192.168.1.1

# Append random data to sent packets (pads payload size to look like normal traffic)
...
# Fast scan (only scans the top 100 most common ports instead of 1,000)
nmap -F 192.168.1.1

# Scan both TCP and UDP ports at the same time in one command
...
# Normal Text Output (exactly what you see on the screen)
nmap -oN scan.txt 192.168.1.1

# Grepable Output (easy to search with command-line tools like grep/awk/cut)
nmap -oG scan.gnmap 192.168.1.1

# XML Output (Required if you want to import results into Metasploit, Zenmap, or Dradis)
nmap -oX scan.xml 192.168.1.1

# Save in ALL THREE major formats at the same time (Highly Recommended)
nmap -oA initial_recon 192.168.1.1


⚡ Quick Reference — Most Used Commands

# Standard recon scan (labs/CTFs)
...
🎯 Production-Ready Aggressive Scan

## When starting a TryHackMe room, HTB machine, or authorized assessment, use this standard aggressive discovery command to capture output quietly:

sudo nmap -sS -sV -O -p- -T4 -oN initial_scan.txt <target_ip>
